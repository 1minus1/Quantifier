...
@synthesize quantifierName, dataSet, csvContents,plot, plotXLocations, plotYLocations,blurredScreenShot,mean, min, max, range, median, sum, dateSinceForPlot, statisticsTitle,selectedRow, hasSegmentedControlInDetailView, type, goal, timeLastUpdatedStepCounter, countOfDataPoints,incrementAmountString, myStepCounter;
...
-(void)updateAutoStepTracker
{
    ...
    NSNumber *timeElapsedSinceLastCallingUpdateAutoStepTracker = [NSNumber numberWithDouble:[now timeIntervalSinceDate:timeLastUpdatedStepCounter]];
    
    if (!timeLastUpdatedStepCounter) {
    }else {
        NSInteger this=[timeElapsedSinceLastCallingUpdateAutoStepTracker integerValue];
        if (this<5 ) {
            NSLog(@"too soon to log steps again.");
            return;
    ...
    timeLastUpdatedStepCounter=now;
    ...
    [components setMinute:0];
    [components setSecond:0];
    NSDate *dayNoon = [calendar dateFromComponents:components];
    ...
    NSLog(@"Date to start: %@",[dayStart descriptionWithLocale:currentLoc]);
    NSLog(@"Date to end step tracking: %@",[now descriptionWithLocale:currentLoc]);
    
    
    
    // Today
    
    NSLog(@"Step tracking avail?: %d",[CMPedometer isStepCountingAvailable]);
    
    if (!myStepCounter) {
        myStepCounter = [CMPedometer new];
    }
    [myStepCounter queryPedometerDataFromDate:dayStart toDate:now withHandler:^(CMPedometerData *pedometerData, NSError *error){
        
        if (error) {
            NSLog(@"steptrackererror");
            return;
            ...
            SZDataPoint *dataPointToAdd =[[SZDataPoint alloc]initWithDataPointValue:@([numberOfSteps integerValue]) date:now valueString:[@([numberOfSteps integerValue])stringValue]];
            
            
            SZDataPoint *dataPointToDelete;
            
            double dataPointToAddValue = [dataPointToAdd.dataPointValue doubleValue];
            ...
            NSInteger nowsDayNumber= [nowComponents day];
            NSInteger nowsMonthNumber= [nowComponents month];
            NSInteger nowsYearNumber = [nowComponents year];
            
            for (SZDataPoint *dataPoint in self.dataSet) {
                double dataPointToDeleteValue=[dataPoint.dataPointValue doubleValue];
                NSDateComponents *thisDatapointsComponents = [[NSCalendar currentCalendar] components:NSCalendarUnitDay | NSCalendarUnitMonth | NSCalendarUnitYear fromDate:dataPoint.timeStamp];
                NSInteger thisDataPointsDayNumber = [thisDatapointsComponents day];
                NSInteger thisDataPointsMonthNumber = [thisDatapointsComponents month];
                NSInteger thisDataPointsYearNumber = [thisDatapointsComponents year];
                
                if (nowsDayNumber==thisDataPointsDayNumber && nowsMonthNumber==thisDataPointsMonthNumber && nowsYearNumber==thisDataPointsYearNumber) {
                    if (dataPointToAddValue>dataPointToDeleteValue) {
                        dataPointToDelete=dataPoint;
                    }
                    if (dataPointToAddValue==dataPointToDeleteValue) {
                        dataPointToDelete=dataPoint;
                    }
            ...
            if (dataPointToDelete) {
                [self.dataSet removeObject:dataPointToDelete];
            }
            if (dataPointToAdd) {
                [self addDataPoint:dataPointToAdd];
                [self sortDataSet];
            }
    ...
    dayStart = [NSDate dateWithTimeInterval:-24*60*60 sinceDate:dayStart];
    dayNoon  = [NSDate dateWithTimeInterval:-24*60*60 sinceDate:dayNoon];
    dayEnd   = [NSDate dateWithTimeInterval:-24*60*60 sinceDate:dayEnd];
    
    [myStepCounter queryPedometerDataFromDate:dayStart toDate:dayEnd withHandler:^(CMPedometerData *pedometerData, NSError *error){
        if (error) {
            return;
        }
            ...
            SZDataPoint *dataPointToAddYest =[[SZDataPoint alloc]initWithDataPointValue:@([numberOfSteps integerValue]) date:dayEnd valueString:[@([numberOfSteps integerValue])stringValue]];

            
            SZDataPoint *dataPointToDelete = [[SZDataPoint alloc] init];
            
            double dataPointToAddValue = [dataPointToAddYest.dataPointValue doubleValue];
            
            NSDateComponents *thisDaysComponents =[[NSCalendar currentCalendar] components:NSCalendarUnitDay | NSCalendarUnitMonth | NSCalendarUnitYear fromDate:dayNoon];
            NSInteger thisDaysDayNumber= [thisDaysComponents day];
            NSInteger thisDaysMonthNumber= [thisDaysComponents month];
            NSInteger thisDaysYearNumber = [thisDaysComponents year];
            ...
            for (SZDataPoint *dataPoint in self.dataSet) {
                ...
                if ( thisDaysDayNumber==thisDataPointsDayNumber && thisDaysMonthNumber==thisDataPointsMonthNumber && thisDaysYearNumber==thisDataPointsYearNumber) {
                    if (dataPointToAddValue>dataPointToDeleteValue) {
                        dataPointToDelete=dataPoint;
                    }
                    if (dataPointToAddValue==dataPointToDeleteValue) {
            ...
            if (dataPointToAddYest) {
                [self addDataPoint:dataPointToAddYest];
                [self sortDataSet];
            }
    ...
    [myStepCounter queryPedometerDataFromDate:dayStart toDate:dayEnd withHandler:^(CMPedometerData *pedometerData, NSError *error){
        if (error) {
            ...
            SZDataPoint *dataPointToAdd3days =[[SZDataPoint alloc]initWithDataPointValue:@([numberOfSteps integerValue]) date:dayEnd valueString:[@([numberOfSteps integerValue])stringValue]];

            SZDataPoint *dataPointToDelete = [[SZDataPoint alloc] init];
            
            double dataPointToAddValue = [dataPointToAdd3days.dataPointValue doubleValue];
            ...
            if (dataPointToAdd3days) {
                [self addDataPoint:dataPointToAdd3days];
                [self sortDataSet];
            }
    ...
    [myStepCounter queryPedometerDataFromDate:dayStart toDate:dayEnd withHandler:^(CMPedometerData *pedometerData, NSError *error){
        ...
        if (error) {
            ...
            SZDataPoint *dataPointToAdd4days =[[SZDataPoint alloc]initWithDataPointValue:@([numberOfSteps integerValue]) date:dayEnd valueString:[@([numberOfSteps integerValue])stringValue]];

            SZDataPoint *dataPointToDelete = [[SZDataPoint alloc] init];
            
            double dataPointToAddValue = [dataPointToAdd4days.dataPointValue doubleValue];
            ...
            if (dataPointToAdd4days) {
                [self addDataPoint:dataPointToAdd4days];
                [self sortDataSet];
            }
    ...
    [myStepCounter queryPedometerDataFromDate:dayStart toDate:dayEnd withHandler:^(CMPedometerData *pedometerData, NSError *error){
        if (error) {
            ...
            SZDataPoint *dataPointToAdd5days =[[SZDataPoint alloc]initWithDataPointValue:@([numberOfSteps integerValue]) date:dayEnd valueString:[@([numberOfSteps integerValue])stringValue]];

            SZDataPoint *dataPointToDelete = [[SZDataPoint alloc] init];
            
            double dataPointToAddValue = [dataPointToAdd5days.dataPointValue doubleValue];
            ...
            if (dataPointToAdd5days) {
                [self addDataPoint:dataPointToAdd5days];
                [self sortDataSet];
            }
    ...
    [myStepCounter queryPedometerDataFromDate:dayStart toDate:dayEnd withHandler:^(CMPedometerData *pedometerData, NSError *error){
        ...
        if (error) {
            ...
            SZDataPoint *dataPointToAdd6days =[[SZDataPoint alloc]initWithDataPointValue:@([numberOfSteps integerValue]) date:dayEnd valueString:[@([numberOfSteps integerValue])stringValue]];

            SZDataPoint *dataPointToDelete = [[SZDataPoint alloc] init];
            
            double dataPointToAddValue = [dataPointToAdd6days.dataPointValue doubleValue];
            ...
            if (dataPointToAdd6days) {
                [self addDataPoint:dataPointToAdd6days];
                [self sortDataSet];
            }
    ...
    [myStepCounter queryPedometerDataFromDate:dayStart toDate:dayEnd withHandler:^(CMPedometerData *pedometerData, NSError *error){
        if (error) {
            ...
            SZDataPoint *dataPointToAdd7days =[[SZDataPoint alloc]initWithDataPointValue:@([numberOfSteps integerValue]) date:dayEnd valueString:[@([numberOfSteps integerValue])stringValue]];

            SZDataPoint *dataPointToDelete = [[SZDataPoint alloc] init];
            
            double dataPointToAddValue = [dataPointToAdd7days.dataPointValue doubleValue];
            ...
            if (dataPointToAdd7days) {
                [self addDataPoint:dataPointToAdd7days];
                [self sortDataSet];
            }
            ///////////////////////////////////////////////////////////////////////////////////////////
            // This line is in the last section of code here, and is not a copy from the previous   ///
            ///////////////////////////////////////////////////////////////////////////////////////////
            
            [[NSNotificationCenter defaultCenter]postNotificationName:@"doneUpdatingStepTracker" object:nil];
            
            [self updateCsvContents];
            [[SZQuantifierStore sharedStore]writeThisQuantifiersCSVToLocalAndDropboxDirectory:self];
